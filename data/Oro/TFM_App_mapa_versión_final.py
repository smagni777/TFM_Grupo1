...
def maximo_activos_por_zoom(zoom):
    """Aumenta el detalle progresivamente al acercarse en el mapa."""
    if zoom >= 16:
        return 1500
    if zoom >= 15:
        return 1000
...
tipo_activo = st.sidebar.multiselect(
    "Activos turísticos",
    options=["Restaurantes", "Puntos de interés (POI)"],
...
with tab2:
    ...
    if "Restaurantes" in tipo_activo:
        restaurantes_busqueda = df_rest_filtrado.copy()
        restaurantes_busqueda["_tipo_busqueda"] = "Restaurante"
    ...
    if (
        ...
        folium.GeoJson(
            ...
            tooltip=folium.GeoJsonTooltip(
                fields=["barrio", "distrito", "densidad_oferta"],
    ...
    capa_restaurantes = folium.FeatureGroup(
        name="Restaurantes",
        show="Restaurantes" in tipo_activo
    )

    cluster_restaurantes = MarkerCluster(
        name="Agrupación de restaurantes",
        control=False,
    ...
    for _, row in restaurantes_mapa.iterrows():
        ...
        nombre_visible = nombre_mapa(row, "Restaurante")
    ...
    ">
        <b>Leyenda</b><br>
        <span style="color:#1967D2;font-size:18px;">●</span>
        Restaurante<br>
    ...
    st.caption(
        ...
        f"{len(restaurantes_candidatos_mapa):,} restaurantes y "
        f"{len(poi_mapa):,} de "
