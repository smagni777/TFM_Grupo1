...
# Versión unificada y corregida contra errores de columnas (KeyError)
# ============================================================
...
# 5. CONTROL Y CÁLCULO DE COLUMNAS DEFENSIVO (EVITA EL ERROR DE COLUMNA FALTANTE)
...
# SEGURO DEFENSIVO: Si tu compañero olvidó meter la columna 'densidad_oferta' en Barrios, la creamos aquí
...
Usa el código con precaución.tarjeta_kpi("Índice Variedad", f"{variedad_val:.2f}/1.0", "Equilibrio y diversificación de oferta", "⚖️")with k4:enfoque_val = df_barrios_filtrado["enfoque_turistico"].mean() if not df_barrios_filtrado.empty else 0tarjeta_kpi("Enfoque Destino", f"{enfoque_val:.1f}%", "Porcentaje de orientación al turista", "🧭")with k5:oportunidad_val = df_barrios_filtrado["indice_oportunidad"].mean() if not df_barrios_filtrado.empty else 0tarjeta_kpi("Potencial IOT", f"{oportunidad_val:.1f}/100", "Capacidad de recibir flujos nuevos", "💡")Pestañas principalestab1, tab2, tab3, tab4 = st.tabs(["📊 Diagnóstico Ejecutivo","🗺️ Cartografía Territorial","🚇 Conectividad y Sostenibilidad","🤖 Algoritmo de Oportunidad"])Pestaña 1 - Diagnósticowith tab1:col_a, col_b = st.columns(2)with col_a:st.markdown("### 🔍 Matriz de Oferta vs Variedad")fig_mix = px.scatter(df_barrios_filtrado,x="enfoque_turistico",y="indice_variedad",size="total_comentarios",hover_name="barrio",color="distrito" if "distrito" in df_barrios_filtrado.columns else None,title="Distribución Territorial (Tamaño de burbuja = Volumen de Reseñas)",labels={"enfoque_turistico": "Enfoque Turístico (%)", "indice_variedad": "Índice de Variedad"})st.plotly_chart(fig_mix, use_container_width=True)with col_b:st.markdown("### 💡 Análisis del Feedback Digital")comentarios_totales_zona = df_barrios_filtrado["total_comentarios"].sum()st.markdown(f"""Integración del Repositorio de Opiniones: Se han procesado de manera ponderada un volumen proyectado de{int(comentarios_totales_zona):,} opiniones sobre los activos de esta de demarcación.Las áreas con alto índice de enfoque y baja variedad representan focos críticos de masificación digital y física.""", unsafe_allow_html=True)st.markdown("#### Top Barrios Recomendados para Desvío de Demanda (Mayor IOT)")if not df_barrios_filtrado.empty:top_barrios = df_barrios_filtrado.sort_values("indice_oportunidad", ascending=False)[["barrio", "indice_variedad", "enfoque_turistico", "indice_oportunidad"]].head(5)st.dataframe(top_barrios.style.format({"indice_variedad": "{:.2f}", "enfoque_turistico": "{:.1f}%", "indice_oportunidad": "{:.1f}"}), use_container_width=True, hide_index=True)Pestaña 2 - Cartografíawith tab2:st.markdown("### 🗺️ Capas Georreferenciadas de Capacidad")mapa = folium.Map(location=[40.4167, -3.7037], zoom_start=13, tiles="CartoDB positron")if not gdf_geo_filtrado.empty and "enfoque_turistico" in df_barrios_filtrado.columns:folium.Choropleth(geo_data=gdf_geo_filtrado,data=df_barrios_filtrado,columns=["barrio", "enfoque_turistico"],key_on="feature.properties.barrio",fill_color="YlOrRd",fill_opacity=0.6,line_opacity=0.3,legend_name="Intensidad de Enfoque Turístico (%)").add_to(mapa)for _, row in df_rest_filtrado.head(300).iterrows():if "lat" in row and "lon" in row and not (pd.isna(row["lat"]) or pd.isna(row["lon"])):folium.CircleMarker(location=[row["lat"], row["lon"]],radius=4,color="#2563eb" if row.get("tiene_terraza") else "#dc2626",fill=True,fill_opacity=0.8,popup=f"{row.get('nombre', 'Activo')}").add_to(mapa)st_folium(mapa, width=1200, height=520)Pestaña 3 - Conectividadwith tab3:st.markdown("### 🚇 Diagnóstico de Movilidad e Infraestructura Abierta")fig_bar = px.bar(df_barrios_filtrado.sort_values("enfoque_turistico", ascending=False),x="barrio",y="enfoque_turistico",color="total_comentarios",title="Orientación de Barrio vs Tráfico y Reseñas",labels={"enfoque_turistico": "Enfoque Turístico (%)", "barrio": "Barrio", "total_comentarios": "Volumen Reseñas"})st.plotly_chart(fig_bar, use_container_width=True)Pestaña 4 - Algoritmowith tab4:st.markdown("### 🤖 Simulación de Alertas Tempranas e Impacto Climático")st.markdown(f"""Alerta de Contexto: Escenario {escenario_clima} (Presión de Flujo: {presion})Bajo una temperatura crítica simulada de {temperatura} °C, los modelos de asignación sugierenpriorizar barrios con alta resiliencia logística ferroviaria y variedad estructural comercial.""", unsafe_allow_html=True)st.markdown("### 🧮 Arquitectura de la Ecuación del Modelo (IOT Avanzado)")st.latex(r"""IOT = 0.30 \cdot A + 0.25 \cdot (1 - D) + 0.25 \cdot V_{ar} + 0.20 \cdot (1 - E_{nf})""")st.markdown("""* A: Accesibilidad Sostenible (Transporte Público a <400m).* D: Concentración de Oferta Espacial (Densidad).* Var: Índice de Variedad de Tipologías (Entropía de Categorías).* Enf: Nivel de Enfoque Turístico (Exposición vs Infraestructura Residente).""")st.markdown("---")st.caption("TUI Territorial Intelligence Dashboard v2.0 • Prototipo Académico de Producción • Desafío 3")