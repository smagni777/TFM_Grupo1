...
h1, h2, h3 { color: var(--tui-navy) !important; letter-spacing: -0.02em; }
h1 { font-weight: 800 !important; }
h2 { font-weight: 750 !important; margin-top: 1.1rem !important; }
h3 { font-weight: 700 !important; }

button[data-baseweb="tab"] { font-weight: 650 !important; color: #667085 !important; padding-top: .75rem !important; padding-bottom: .75rem !important; }
button[data-baseweb="tab"][aria-selected="true"] { color: var(--tui-red) !important; }
div[data-baseweb="tab-highlight"] { background-color: var(--tui-red) !important; height: 3px !important; }

.madrid-banner {
    position: relative; border-radius: 18px; overflow: hidden;
    box-shadow: 0 8px 28px rgba(27,17,91,.14); margin: 0 0 22px 0;
    height: 300px; background: #10142B;
}
.madrid-banner-grid { position: absolute; inset: 0; display: flex; width: 100%; height: 100%; }
.madrid-banner-grid img {
    flex: 1 1 0; width: 100%; height: 100%; object-fit: cover; display: block;
    filter: saturate(.92) contrast(1.04);
}
.madrid-banner-overlay {
    position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: center;
    gap: 9px; padding: 34px 5%; max-width: 72%;
    background: linear-gradient(90deg, rgba(11,16,40,.97) 0%, rgba(11,16,40,.90) 34%, rgba(11,16,40,.62) 64%, rgba(11,16,40,.08) 100%);
}
.tui-hero-brand {
    display: inline-flex; align-items: center; width: fit-content; background: var(--tui-red);
    color: #FFFFFF !important; border-radius: 999px; padding: 6px 13px; font-size: 13px;
    font-weight: 800; letter-spacing: .06em; text-transform: uppercase;
    box-shadow: 0 4px 12px rgba(0,0,0,.18);
}
.madrid-banner-overlay h1 {
    color: #FFFFFF !important; font-size: 38px; font-weight: 800; margin: 0; line-height: 1.08;
    text-shadow: 0 2px 12px rgba(0,0,0,.28);
}
.madrid-banner-overlay h3 {
    color: #FFFFFF !important; font-size: 17px; font-weight: 500; margin: 0; line-height: 1.45; max-width: 760px;
}
.madrid-banner-overlay p {
    color: #DCEBFF !important; font-size: 13px; font-weight: 650; letter-spacing: .03em; margin: 4px 0 0 0;
}
.madrid-banner-tile-vacio { flex: 1 1 0; width: 100%; height: 100%; background: #1B115B; }

@media (max-width: 760px) {
    .madrid-banner { height: 245px; }
    .madrid-banner-overlay { max-width: 100%; padding: 24px 6%; background: linear-gradient(90deg, rgba(11,16,40,.96), rgba(11,16,40,.70)); }
    .madrid-banner-overlay h1 { font-size: 27px; }
    .madrid-banner-overlay h3 { font-size: 14px; }
}

.kpi-card {
    background: var(--tui-card); padding: 17px 18px 16px 18px; border-radius: 14px;
...
.kpi-value { color: var(--tui-navy) !important; font-size: 28px; line-height: 1.1; font-weight: 800; }
.kpi-description { color: #98A2B3 !important; font-size: 11px; margin-top: 7px; line-height: 1.35; }

.insight-box, .opportunity-box, .warning-box {
    background: #FFFFFF !important; color: var(--tui-ink) !important; padding: 17px 18px;
    border-radius: 12px; margin-bottom: 12px; box-shadow: 0 2px 10px rgba(16,24,40,.045);
    border: 1px solid var(--tui-border);
}
.insight-box { border-left: 4px solid var(--tui-blue); }
.opportunity-box { border-left: 4px solid #2E8B57; }
.warning-box { border-left: 4px solid #E0A800; }

.level-box { padding: 18px 20px; border-radius: 12px; border-left: 5px solid; box-shadow: 0 2px 10px rgba(16,24,40,.04); }
.level-box h3 { margin-top: 0; font-size: 16px; text-transform: uppercase; letter-spacing: .03em; }
.level-nominal { background: #EEF8F1; border-left-color: #2E8B57; }
.level-moderate { background: #FFF8E6; border-left-color: #E0A800; }
.level-elevated { background: #FDEEE0; border-left-color: #D9722C; }
.level-critical { background: #FBE9E7; border-left-color: #C0392B; }

div[data-testid="stMetric"] {
    background: #FFFFFF; border: 1px solid var(--tui-border); border-radius: 12px;
    padding: 12px 14px; box-shadow: 0 2px 8px rgba(16,24,40,.04);
}
div[data-testid="stMetricLabel"] { color: #667085 !important; }
div[data-testid="stMetricValue"] { color: var(--tui-navy) !important; font-weight: 800 !important; }

div[data-testid="stDataFrame"] { border: 1px solid var(--tui-border); border-radius: 12px; overflow: hidden; }
.stButton > button { border-radius: 9px; border: 1px solid #D0D5DD; font-weight: 650; }
.stButton > button:hover { border-color: var(--tui-red); color: var(--tui-red); }
div[data-baseweb="select"] > div { border-radius: 9px !important; border-color: #D0D5DD !important; }
.section-divider { margin-top: 20px; margin-bottom: 20px; border-top: 1px solid var(--tui-border); }
.muted { color: #667085 !important; font-size: 14px; }
.tui-footer { text-align: center; color: #98A2B3; font-size: 11px; padding: 20px 0 4px 0; }
...
def tarjeta_kpi(titulo, valor, descripcion):
    """Genera una tarjeta KPI con formato sobrio, sin iconografía decorativa."""
    st.markdown(
        f"""
        <div class="kpi-card">
...
def estilo_figura_tui(fig, titulo=None, altura=410):
    """Aplica un sistema visual homogéneo a las gráficas del dashboard."""
    if titulo:
    ...
    fig.update_layout(
        ...
        title=dict(font=dict(family="Arial, sans-serif", size=17, color="#1B115B"),
                   x=0, xanchor="left", y=.98, yanchor="top"),
        margin=dict(l=52, r=22, t=58, b=70),
        hoverlabel=dict(bgcolor="#1B115B", font=dict(color="#FFFFFF", size=12)),
        legend=dict(orientation="h", yanchor="bottom", y=1.01, xanchor="right", x=1),
        xaxis=dict(showgrid=False, linecolor="#E6EAF0", tickfont=dict(color="#667085", size=11),
                   title_font=dict(color="#475467", size=12)),
        yaxis=dict(showgrid=True, gridcolor="#EEF1F5", zeroline=False, linecolor="#E6EAF0",
                   tickfont=dict(color="#667085", size=11), title_font=dict(color="#475467", size=12))
    )
    return fig
...
def obtener_clima_actual(latitud=40.4167, longitud=-3.7037):
    """
    Integración de datos abiertos externos (Open-Meteo, API pública
    sin clave de acceso) para obtener la temperatura real vigente.

    Devuelve un diccionario con la temperatura y la hora de
    observación, o None si el servicio no está disponible.
    """
    try:
        ...
        if "temperature" not in datos:
            return None

        return {
            "temperatura": float(datos["temperature"]),
            "hora_observacion": datos.get("time"),
...
st.sidebar.markdown("### 4. Características de oferta")
...
st.sidebar.markdown("### 5. Escenario contextual")

fuente_temperatura = st.sidebar.radio(
    "Fuente de la temperatura",
    options=["Simulación manual", "Dato meteorológico real (Madrid)"],
    help="""
    La simulación manual permite explorar escenarios
    hipotéticos. La opción de dato real consulta la API
    abierta Open-Meteo para obtener la temperatura vigente
    en Madrid en el momento de la consulta.
    """
)

clima_real = None

if fuente_temperatura == "Dato meteorológico real (Madrid)":
    clima_real = obtener_clima_actual()

if clima_real is not None:

    temperatura = round(clima_real["temperatura"])

    st.sidebar.caption(
        f"Temperatura real observada: {clima_real['temperatura']:.1f} °C "
        f"(Open-Meteo, {clima_real['hora_observacion']})"
    )
    ...
    if fuente_temperatura == "Dato meteorológico real (Madrid)":
        st.sidebar.warning(
            ...
            "Se utiliza la simulación manual como alternativa."
        )
...
def _imagen_base64(nombre_archivo):
    """
    Lee una imagen local de la carpeta /images y la devuelve como
    data URI en base64, lista para insertar en un <img src="...">.
    Devuelve None si el archivo no existe todavía, para que el
    dashboard no falle si alguna foto no se ha subido al repositorio.
    """
    ruta = IMAGES_DIR / nombre_archivo
    ...
    datos = base64.b64encode(ruta.read_bytes()).decode("utf-8")
...
def banner_madrid():
    ...
    for nombre_archivo, alt in fotos:
        data_uri = _imagen_base64(nombre_archivo)

        if data_uri:
            tiles_html += f'<img src="{data_uri}" alt="{alt}" title="{alt}">'
        else:
            faltan.append(nombre_archivo)
            tiles_html += (
                f'<div class="madrid-banner-tile-vacio" title="{alt}"></div>'
            )

    st.markdown(
        f"""
        <div class="madrid-banner">
            <div class="madrid-banner-grid">{tiles_html}</div>
            <div class="madrid-banner-overlay">
                <div class="tui-hero-brand">TUI · DESTINATION INTELLIGENCE</div>
                <h1>Territorial Intelligence Dashboard</h1>
                <h3>Inteligencia georreferenciada para comprender la oferta turística de Madrid</h3>
                <p>DESTINO · OFERTA · ACCESIBILIDAD · OPORTUNIDADES</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if faltan:
        st.caption(
            "Faltan imágenes en la carpeta /images: " + ", ".join(faltan)
        )
...
with c1:
    ...
    st.metric(
        ...
        f"{temperatura} °C",
        help=(
            "Dato meteorológico real (Open-Meteo)"
            if clima_real is not None
            else "Valor introducido mediante simulación manual"
        )
    )
...
st.caption(
    """
    La temperatura procede de un dato meteorológico real
    (Open-Meteo) o de una simulación manual, según la opción
    seleccionada en el panel lateral. En ambos casos se emplea
    como variable contextual y no constituye, por sí misma,
    una demostración de causalidad sobre la congestión turística.
    """
)
...
with tab3:
    ...
    with col1:
        ...
        if (
            ...
            fig_densidad.update_traces(
                marker_color="#D50D15",
                marker_line_width=0,
                hovertemplate="<b>%{x}</b><br>Densidad: %{y:.1f}<extra></extra>"
            )
            estilo_figura_tui(fig_densidad, "Densidad de negocios por barrio", 420)
            fig_densidad.update_layout(
                xaxis_tickangle=-38,
                xaxis_title="Barrio",
                yaxis_title="Negocios por km²"
            )
    ...
    with col2:
        ...
        if (
            ...
            fig_scatter.update_traces(
                marker=dict(color="#6FCBF4", line=dict(color="#1B115B", width=1), opacity=.86),
                hovertemplate="<b>%{hovertext}</b><br>Accesibilidad: %{x:.1f}<br>Densidad: %{y:.1f}<extra></extra>"
            )
            estilo_figura_tui(fig_scatter, "Accesibilidad vs densidad de oferta", 420)
            fig_scatter.update_layout(
                xaxis_title="Accesibilidad · paradas próximas",
                yaxis_title="Densidad · negocios por km²"
            )
...
with tab4:
    ...
    if (
        ...
        fig_op.update_traces(
            marker_color="#1B115B",
            marker_line_width=0,
            hovertemplate="<b>%{x}</b><br>Índice: %{y:.1f}/100<extra></extra>"
        )
        estilo_figura_tui(fig_op, "Índice de oportunidad territorial", 430)
        ...
        fig_op.add_hline(
            y=70, line_dash="dot", line_color="#D50D15", line_width=2,
            annotation_text="Umbral orientativo · 70", annotation_position="top right",
            annotation_font=dict(color="#D50D15", size=11)
        )
...
with tab5:
    ...
    st.markdown(
        f"""
        <div class="level-box {clase_nivel}">
    ...
    st.info(
        ...
        **Nota metodológica**
        ...
        Este simulador utiliza la temperatura como variable
        contextual hipotética, con independencia de que el valor
        de partida proceda del dato meteorológico real (Open-Meteo)
        o de una simulación manual. El sistema no afirma una
        relación causal comprobada entre temperatura y congestión
        turística; el objetivo es explorar escenarios de sensibilidad.

        Próximas iteraciones podrían incorporar series históricas
        de temperatura, ocupación real y datos de movilidad para
        calibrar el modelo con evidencia empírica longitudinal.
        """
    )
...
def cargar_modelo_recomendacion():
    return pd.read_parquet(
        "data/Oro/Modelo_Recomendacion_Recursos.parquet"
    )
...
st.markdown(
    ...
    <div class="tui-footer">
        TUI Territorial Intelligence · Reto 3 · Prototipo académico ·
